...
本仓库（**LingFan.Media**，以 Apache2.0 许可证分发）是**跨全端**媒体基础设施层，目标平台覆盖
**Windows / Linux / macOS / Android / iOS**。在上述全平台上均可通过 **FFmpeg** 或 **VLC（LibVLC）**
提供媒体解封装与解码能力（二者均为可选的播放后端，由回退中间件按注册顺序自动选择）；此外在 **Windows**
上还支持 **Media Foundation**（Windows 操作系统内置组件，无需单独分发的第三方授权）。为遵守各自的 **LGPL**，
本项目**仅以动态链接方式**使用 FFmpeg / LibVLC 的共享库（Windows 为 `.dll`、Linux / Android 为 `.so`、
macOS / iOS 为 `.dylib`），**绝不**将它们的源码或静态库合并进本项目的任何程序集。
...
- **动态链接与跨全端说明**：本项目通过 `FFmpeg.AutoGen` 的 `DynamicallyLoaded` 绑定在**运行时**动态加载
  FFmpeg 共享库——Windows 为 `avcodec-*.dll` 等、Linux / Android 为 `libavcodec.so*` 等、
  macOS / iOS 为 `libavcodec.*.dylib` 等。FFmpeg 的 **LGPL 共享构建在全部目标平台可用**：
  Windows 采用 BtbN `lgpl-shared`（见下）；Linux / macOS / Android / iOS 采用各自平台对应的 LGPL 共享构建
...
- **动态链接说明（跨全端）**：本项目经由 `LibVLCSharp`（LGPLv2.1+）在**运行时**动态加载 libVLC 共享库——Windows 为 `libvlc.dll` / `libvlccore.dll`、Linux / Android 为 `libvlc.so` / `libvlccore.so`、macOS / iOS 为 `libvlc.dylib` / `libvlccore.dylib`（iOS 以动态 framework 形式）。原生二进制由对应平台的 `VideoLAN.LibVLC.*` NuGet 提供（按 RID 分发 Windows / Linux / macOS / **Android / iOS**），部署时落在 `bin/<tfm>/libvlc/<rid>/`，且已被 `.gitignore` 忽略（不随源码提交、不内嵌）。
...
| 后端 | 授权 | 平台 | 说明 |
| --- | --- | --- | --- |
| FFmpeg | LGPL 2.1+（共享构建） | Windows / Linux / macOS / **Android / iOS** | 跨全端主后端，经 `FFmpeg.AutoGen` 动态链接 |
| LibVLC / VLC | LGPL 2.1+（引擎+播放模块+移动端绑定） | Windows / Linux / macOS / **Android / iOS** | 备用后端，经 `LibVLCSharp` 动态链接（移动端亦对应经 VLCKit / libvlcjni 调用） |
