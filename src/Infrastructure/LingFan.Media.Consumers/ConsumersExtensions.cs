...
public static class ConsumersExtensions
{
    ...
    /// 替代 <c>AddD3D11Renderer()</c> / <c>AddVulkanRenderer()</c> 等具体渲染器注册。
    /// 配合 <c>player.VideoFrameAvailable</c> 订阅或 <see cref="ProcessingFrameSink"/> 实现无头帧消费。
    /// </summary>
    ...
    /// 与 <see cref="AddHeadlessRenderer"/> 对称。注意：本方法只是把音频采样丢弃（NoOp），
    /// 与"宿主有没有音频设备"无关——无头进程若有真实音频设备、想让声音真的播放出来，应改用 <c>AddWasapiOutput()</c>
    /// （WASAPI 不依赖窗口，无头服务 / 控制台进程同样可用）。
    /// 配合 <see cref="ProcessingFrameSink"/> 实现无头帧消费。
    /// </summary>
    /// <param name="builder">媒体构建器。</param>
    /// <returns>构建器（链式调用）。</returns>
    /// <remarks>此方法为同步配置（config 分类），无 I/O。</remarks>
