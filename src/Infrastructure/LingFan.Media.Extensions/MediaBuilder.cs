...
public sealed class MediaBuilder
{
    ...
    /// 透传至 Core <c>AudioPipeline</c>。null = 不注入。
    /// </summary>
    ...
    /// 供 Core <c>AudioPipeline</c> 在 Seek/Flush 解码锁内调用。null = 不注入。
    /// </summary>
    ...
    /// 透传至 Core <c>VideoPipeline</c>。null = 不注入。
    /// </summary>
    ...
    /// 供 Core <c>VideoPipeline</c> 在 Seek/Flush 解码锁内调用。null = 不注入。
    /// </summary>
    ...
    /// <summary>
    /// 注入音频管线配置，将效果链与重置钩子透传至 Core 音频管线。
    /// </summary>
    ...
    /// <summary>
    /// 注入视频管线配置，将后处理链与重置钩子透传至 Core 视频管线。
    /// </summary>
    /// <param name="config">视频管线配置（含后处理链）。</param>
    /// <returns>同一构建器，便于链式调用。</returns>
    /// <remarks>
    /// 不调用本方法则视频后处理与重置钩子为 null。
