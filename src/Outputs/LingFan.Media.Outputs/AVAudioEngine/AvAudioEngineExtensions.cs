...
/// <summary>
/// AVAudioEngine 音频输出 DI 注册扩展方法（桩）。
/// </summary>
/// <remarks>
/// <para>使用模式：<code>services.AddLingFanMedia().AddAvAudioEngineOutput()</code></para>
/// <para>注册 <see cref="AvAudioEngineOutputFactory"/> 为 Singleton。
/// 调用 <c>Create()</c> 返回的 <see cref="AvAudioEngineOutput"/> 在使用时抛出 <see cref="NotSupportedException"/>。</para>
/// <para>此方法为同步配置（config 分类），无 I/O。</para>
/// </remarks>
public static class AvAudioEngineExtensions
{
    /// <summary>
    /// 注册 AVAudioEngine 音频输出（桩——尚未实现）。
    /// </summary>
