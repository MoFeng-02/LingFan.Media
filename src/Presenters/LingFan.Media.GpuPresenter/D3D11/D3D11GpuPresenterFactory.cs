...
/// </remarks>
public sealed class D3D11GpuPresenterFactory : IGpuPresenterFactory
{
    private readonly IVideoRendererFactory _rendererFactory;
    private readonly ILoggerFactory _loggerFactory;

    /// <summary>
    /// 初始化 <see cref="D3D11GpuPresenterFactory"/> 的新实例。
    /// </summary>
    /// <param name="rendererFactory">视频渲染器工厂（DI 注入）。</param>
    /// <param name="loggerFactory">日志工厂。</param>
    public D3D11GpuPresenterFactory(IVideoRendererFactory rendererFactory, ILoggerFactory loggerFactory)
    {
    ...
    /// <inheritdoc/>
    public IGpuPresenter Create() => new D3D11GpuPresenter(_rendererFactory, _loggerFactory);
}
