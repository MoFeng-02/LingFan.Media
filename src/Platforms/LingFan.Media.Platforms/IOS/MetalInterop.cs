...
/// <summary>
/// iOS Metal GPU 互操作。V1 桩实现。
/// </summary>
/// <remarks>
/// <para>职责：将 CVPixelBuffer 直接导入 Metal 纹理（MTLTexture），
...
/// 可直接传入 <c>MTLTexture</c> 创建方法（无需显式 IOSurface 步骤）。</para>
/// <para>V1 桩——所有方法抛出 <see cref="NotSupportedException"/>。
/// Metal 互操作属 Phase 2-3 目标（iOS）。
...
{
    /// <summary>
    /// 从 CVPixelBuffer 创建 Metal 纹理（iOS 直接路径，无需 IOSurface 中间步骤）。
    /// </summary>
    /// <param name="metalDevice">id&lt;MTLDevice&gt; 句柄。</param>
    /// <param name="pixelBuffer">CVPixelBuffer 句柄（来自 VideoToolbox 解压输出）。</param>
    ...
    public nint CreateTextureFromCVPixelBuffer(
        ...
        => throw new NotSupportedException("iOS Metal 互操作尚未实现。Phase 2-3 目标。");
    ...
    public nint CreateMetalLayer(int width, int height)
        => throw new NotSupportedException("iOS CAMetalLayer 创建尚未实现。");

    /// <summary>
    /// 获取 Metal 设备（id&lt;MTLDevice&gt;）。
    /// </summary>
    /// <returns>id&lt;MTLDevice&gt; 句柄。</returns>
    public nint GetMetalDevice()
        => throw new NotSupportedException("iOS Metal 设备获取尚未实现。");
}
