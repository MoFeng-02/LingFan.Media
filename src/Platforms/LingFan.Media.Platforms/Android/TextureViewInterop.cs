...
/// <c>TextureView.SurfaceTexture → Surface → ANativeWindow</c> 得到的指针）传入的
/// <c>ANativeWindow*</c>，封装 NDK 原生窗口操作（acquire / release / setBuffersGeometry），
/// 供 Vulkan/EGL SwapChain 绑定，由 SurfaceFlinger 合成到 View 树（无独立窗口，无空域）。</para>
/// <para><b>范围说明</b>：本库为 net10.0，不能引用 Android Java 类型（Surface/SurfaceTexture），
...
public sealed unsafe partial class TextureViewInterop : IDisposable
{
    ...
    private static void ThrowIfNotAndroid()
    {
        if (!OperatingSystem.IsAndroid())
            throw new PlatformNotSupportedException("TextureView 互操作仅支持 Android。");
    }
}
