...
/// 实际硬解路径由 FFmpeg + D3D11VA 提供（在 Backends.FFmpeg 模块实现），不经过此接口。</para>
...
public sealed class WindowsPlatformServices : IPlatformServices
{
    ...
    public IVideoDecoder? CreateHardwareDecoder(VideoCodec codec)
        => throw new NotSupportedException(
            "Windows 硬件解码尚未通过 IPlatformServices 实现。硬解路径由 FFmpeg + D3D11VA 提供。");

    /// <inheritdoc/>
    public object? GetGPUContext(GPUApiType type)
        => throw new NotSupportedException(
            "Windows GPU 上下文尚未通过 IPlatformServices 实现。由 D3D11RendererFactory 直接创建 ID3D11Device。");
}
