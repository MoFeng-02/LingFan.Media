...
namespace OpenGLHeadfulPlaybackProbe;

/// <summary>
/// 有头（Headful）全链路验证程序：真实 OpenGL（WGL）SwapChain 上屏（GPU Present）与真实 WASAPI 出声，
...
///   <item><c>--no-audio</c>：只验 OpenGL 上屏。</item>
...
internal static class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            // （125%/150%）非整数倍拉伸到物理像素，这层拉伸发生在 GL 之外，
    ...
    private static async Task<int> RunAsync(string[] args)
    {
        ...
        if (fullPlayback)
        {
            ...
            // 音频播放时钟：让 WasapiRenderLoop 校准播放位置。建议常开。
        ...
        // 用 --window-w / --window-h 指定与源同尺寸可做 1:1 对照（GL 着色器按 attached 尺寸做 GPU 缩放）。
        int windowW = (int)ParseDouble(args, "--window-w", DefaultWindowW);
        int windowH = (int)ParseDouble(args, "--window-h", DefaultWindowH);
        // 宽高比缩放模式：--scale fill|uniform|uniformtofill（默认 uniform，保持比例留黑边）
        AspectRatioMode scaleMode = ParseOption(args, "--scale") switch
        {
        ...
        {
            // 恒软解：OpenGL 走「CPU 软解帧 → GL 纹理上传 → 着色器上屏」路径，
            // 无法消费 D3D11 DXVA 纹理，故此处强制软件解码（EnableHardwareDecoding/EnableDxva=false），
            // 使帧恒为 CPU SoftwareFrameResource，OpenGL 可上传上屏；不注册 FFmpeg/VP9-AV1 回退
        ...
        try
        {
            ...
            if (doVideo)
            {
                // 真实可见窗口：专用 STA 线程 + 消息泵（WGL SwapBuffers 需要有效 HWND + 消息循环）。
                // 加 --visible 可肉眼确认上屏。
            ...
            if (doVideo && win is not null && win.Hwnd != IntPtr.Zero)
            {
                if (countingFactory!.Last is null)
                    Console.WriteLine("OpenGL 渲染器未创建（环境无 GPU/显示或 GL 不可用），跳过视频有头验证。");
                else
                    // 用「实测客户区」而非构造入参：两者不等时按入参建上下文会让 DWM 再叠一层拉伸。
            ...
            if (doVideo)
            {
                ...
                Console.WriteLine($"[HEADFUL-VIDEO] glPresentCount={presentCount}  => " +
            ...
            if (replayTest)
            {
                ...
                // 无视频用例跳过呈现计数判定
                bool replayPresentOk = !doVideo || replayPresentDelta >= 5;
    ...
    private sealed class CountingVideoRenderer : IVideoRenderer
    {
        ...
        public void Present(VideoFrame frame)
        {
            ...
            // CPU 软解帧走 NV12/BGRA/RGBA 转换；OpenGL 不消费 GPU 纹理帧，无需 Readback 分支。
            if (_saveFrames > 0 && n % _saveFrames == 0)
                FrameDumper.DumpFrame(frame, n, _saveDir);
            _inner.Present(frame);
        }
        public TimeSpan PresentationLatency => _inner.PresentationLatency;
    ...
    private sealed class RenderWindow : IDisposable
    {
        ...
        /// <summary>窗口真实客户区宽（GetClientRect 实测，不是构造入参）。GL 上下文必须按它建，否则 DWM 会再拉伸一层。</summary>
        ...
        private const string WindowClassName = "LingFanGlProbeWnd";
        ...
        public RenderWindow(int w, int h, bool visible)
        {
            ...
            _thread.SetApartmentState(ApartmentState.STA); // WGL 依赖 STA 线程 + 消息泵
            _thread.Start();
        ...
        private void Run()
        {
            ...
            // 实测客户区与 DPI：Attach 必须用实测客户区，用构造入参会让 GL 上下文与窗口错位，DWM 会再拉伸一层。
...
// 帧落盘诊断：把渲染器收到的真实帧（CPU 软解）转 RGBA 后写极简 PNG。
...
internal static class FrameDumper
{
    ...
    internal static void DumpFrame(VideoFrame frame, int presentIndex, string dir)
    {
        try
        {
            ...
            {
                Console.WriteLine($"  [HEADFUL-SAVE] 帧 {presentIndex} 资源类型 {frame.Resource?.GetType().Name ?? "null"} 不可落盘（OpenGL 仅消费 SoftwareFrameResource），跳过");
                return;
            }
