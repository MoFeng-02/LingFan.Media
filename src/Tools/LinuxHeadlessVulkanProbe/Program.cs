...
internal static class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        ...
        // 跨平台：ffmpeg 解码 + Vulkan 渲染器(X11 Surface) + 音频输出（openal 真实出声 / silent 数据出餐）。
        ...
        Console.Error.WriteLine("[VULKAN-INIT] 工厂已解析，创建渲染器…");
        IVideoRenderer renderer = factory.Create();
        Console.Error.WriteLine("[VULKAN-INIT] 渲染器已创建，Attach X11 窗口…");
        renderer.Attach(new X11RenderTarget(new X11WindowHandle(display, window), winW, winH));
        Console.Error.WriteLine("[VULKAN-INIT] Attach 完成");
