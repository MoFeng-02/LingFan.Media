...
namespace LinuxHeadlessOpenGlProbe;

/// <summary>
/// Linux/WSL2 无头 OpenGL 渲染探针（Phase 2a）：在 Xvfb 虚拟显示提供的 X11 窗口上，
/// 用 OpenGLRenderer(EGL X11) 真上屏 present，统计 glPresentCount / GPU 纹理帧 / CPU 帧 / 丢帧。
/// 证明「ffmpeg 解码 → OpenGL 渲染 present」在 Linux 无头环境（无物理显示器、非 WSLg GUI）可闭环。
...
internal static class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        ...
        {
            Console.WriteLine("本探针仅适用于 Linux/WSL2（依赖 libX11 + EGL X11）。");
            return 1;
        }
        ...
        var factory = sp.GetRequiredService<OpenGLRendererFactory>();
        ...
        Console.WriteLine($"GL present 次数 : {presentCount}");
        Console.WriteLine($"GPU 纹理帧      : {gpuServed}   CPU 内存帧: {cpuServed}");
        Console.WriteLine($"判读            : 资源=GPU + 渲染路径=零拷贝 ⇒ 硬解GPU纹理直投GL(零拷贝)；资源=CPU ⇒ 软解软渲；每 {sample} 帧抽一帧打印");
        Console.WriteLine($"音频回调        : {audioCallbacks}  采样数: {audioSamples}  采样率: {sampleRate}");
        Console.WriteLine($"丢帧            : {player.VideoDroppedFrames}");
        if (presentCount > 0)
            Console.WriteLine("判定            : OpenGL 在 Linux 无头(EGL X11)成功 present——渲染闭环通");
        else
            Console.WriteLine("判定            : 异常——未见 GL present，请检查上方 EGL/X11 日志与 Xvfb 是否运行");
        return 0;
    }
