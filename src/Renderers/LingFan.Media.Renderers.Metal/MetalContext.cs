...
/// 本上下文不提供离屏设备上下文（GPU 纹理零拷贝属 C 线未来增强，故不注册 <see cref="LingFan.Media.Abstractions.IGpuDeviceContext"/>）。</para>
    /// <para><b>所有权</b>：device 来源有二——复用宿主图层既有 device（本层另 <see cref="MetalNative.objc_retain"/> 取 +1）或回退 <c>MTLCreateSystemDefaultDevice</c>（本层所有 +1）；
    /// queue(来自 newCommandQueue) 均按 Cocoa 规则返回 +1（本层所有）。二者均在 <see cref="Dispose"/> 中 <see cref="MetalNative.objc_release"/> 一次即平衡（无需额外 retain）；
    /// layer 为宿主借入对象，经 <see cref="MetalNative.objc_retain"/> 取得本层 +1，<see cref="Dispose"/> 释放。CAMetalLayer 同时被宿主视图（NSView/UIView）强引用，释放仅解除本层引用。</para>
...
internal sealed class MetalContext : IDisposable
{
    ...
    internal MetalContext(nint layer, int width, int height, ILogger? logger = null)
    {
        ...
        _logger = logger;
        _layer = layer;
        MetalNative.objc_retain(_layer);
        ...
        nint hostDevice = MetalNative.objc_msgSend(_layer, MetalNative.Sel("device"));
        if (hostDevice != nint.Zero)
        {
            // 图层已持有该 device（+1）；本层另取自有 +1，Dispose 释放一次即平衡，且不覆盖宿主选择。
            _device = hostDevice;
            MetalNative.objc_retain(_device);
        }
        else
        {
            ...
            MetalNative.objc_msgSend(_layer, MetalNative.Sel("setDevice:"), _device);
        }

        // 配置 CAMetalLayer：渲染目标像素格式固定 BGRA8Unorm（与着色器输出一致）。device 已就绪（复用宿主或回退默认）。
        MetalNative.objc_msgSend(_layer, MetalNative.Sel("setPixelFormat:"), MetalConstants.BGRA8Unorm);
        MetalNative.objc_msgSend(_layer, MetalNative.Sel("setOpaque:"), (byte)1);
        SetDrawableSize(width, height);

        _queue = MetalNative.objc_msgSend(_device, MetalNative.Sel("newCommandQueue"));
        if (_queue == nint.Zero)
    ...
    {
        MetalNative.objc_msgSend(_layer, MetalNative.Sel("setDrawableSize:"), (double)width, (double)height);
    }
    ...
    {
        nint drawable = MetalNative.objc_msgSend(_layer, MetalNative.Sel("nextDrawable"));
        if (drawable == nint.Zero)
            throw new InvalidOperationException("Metal：nextDrawable 返回 null（图层不可见、被移出窗口或设备丢失）。");

        nint texture = MetalNative.objc_msgSend(drawable, MetalNative.Sel("texture"));
        if (texture == nint.Zero)
            throw new InvalidOperationException("Metal：可绘制层纹理为 null（图层尺寸为 0，请确认 drawableSize 已设置）。");

        int w = (int)MetalNative.objc_msgSend(texture, MetalNative.Sel("width"));
        int h = (int)MetalNative.objc_msgSend(texture, MetalNative.Sel("height"));
    ...
    public void Dispose()
    {
        ...
        if (_queue != nint.Zero)
        {
            MetalNative.objc_release(_queue);
            _queue = nint.Zero;
        }
        if (_device != nint.Zero)
        {
            MetalNative.objc_release(_device);
            _device = nint.Zero;
        }
        if (_layer != nint.Zero)
        {
            MetalNative.objc_release(_layer);
            _layer = nint.Zero;
        }
