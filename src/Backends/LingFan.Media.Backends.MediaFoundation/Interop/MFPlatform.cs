...
/// </summary>
/// <remarks>
/// <para><b>为什么需要</b>：<c>MFStartup</c>/<c>MFShutdown</c> 是进程级全局 API——
...
/// 与 <c>MFDemuxer</c>（解封装侧，OpenCore Startup / ReleaseNativeResources Shutdown；后续补入——
...
/// 原生访问违规（ACCESS_VIOLATION）→ <c>COR_E_EXECUTIONENGINE</c>（原生堆损坏）非确定性崩溃
/// （MF 冷启动偶发崩溃的成因，<c>AssemblyInfo</c> 的 <c>DisableTestParallelization</c> 只防跨测试、防不住此竞态）。</para>
...
internal static class MFPlatform
{
    ...
    /// 殃及其它组件仍持有的 COM 对象。<c>MFDemuxer</c> 的 COM 单元不变量（Release 先于 CoUninitialize、且同线程）解决了
    ...
    internal static void Startup()
    {
        lock (_gate)
        {
            if (_refCount == 0)
            {
                ...
                // 失败不致命（旧系统或极端情形）——退化为原行为，由 COM 单元不变量单独保证 MFDemuxer 自身安全。
