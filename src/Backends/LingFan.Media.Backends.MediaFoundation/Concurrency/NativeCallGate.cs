...
/// </summary>
/// <remarks>
/// <para>本类是 MF 冷启动 <c>0x80131506</c>（<c>COR_E_EXECUTIONENGINE</c>，原生堆损坏→CLR 自杀）修复的核心构件。
...
/// <list type="bullet">
/// <item>I1 单调性：<see cref="BeginClose"/> 后 <see cref="TryEnter"/> 恒返回 false ⇒ 在途计数单调不增。</item>
/// <item>I2 稳定性：由 I1，<c>_inFlight==0 &amp;&amp; _closing</c> 一旦成立便永久成立，故 WaitDrain 返回 true 是稳定结论（无 ABA）。</item>
/// <item>I3 独占性：由 I2，WaitDrain==true 之后不存在任何线程处于闸内 ⇒ 闸内引用资源可由关闭线程独占释放。</item>
/// <item>I4 安全侧默认：WaitDrain==false 时不满足 I3 ⇒ 不释放（只泄漏不崩）。</item>
/// <item>I5 无死锁：<see cref="Exit"/> 在 lock 内仅 PulseAll + TrySetResult；TCS 用 RunContinuationsAsynchronously，续体不在 lock 内执行。</item>
/// <item>I6 配对性：TryEnter/Exit 必须 try/finally 严格配对。<b>两种失配的后果并不对称</b>——
/// <b>漏 Exit</b>：计数永不归零 ⇒ drain 超时 ⇒ 有意泄漏，**安全侧**失败；
/// <b>多 Exit</b>：把他人的计数减掉 ⇒ 可能<b>提前</b>判定排空 ⇒ 释放仍在使用的指针 ⇒ use-after-free，**危险侧**失败。
/// 故 I3 的成立以「不存在多余 Exit」为前提，该前提由调用点穷举审计保证（详见 <see cref="Exit"/> 备注）。</item>
/// </list>
...
internal sealed class NativeCallGate
{
    ...
    private readonly TaskCompletionSource _drained =
        new(TaskCreationOptions.RunContinuationsAsynchronously); // 必须异步续体，防在 lock 内跑续体（I5）
    ...
    /// 须与 <see cref="Exit"/> 严格 try/finally 配对（I6）。
    /// </summary>
    ...
    /// <summary>
    /// 离开原生调用区。必须与 <see cref="TryEnter"/>==true 严格 try/finally 配对（I6）。
    /// </summary>
    /// <remarks>
    /// <para>多余 Exit（未 Enter 即 Exit）被钳制为 no-op 以避免计数下溢，但钳制<b>不能</b>消除其危害：
    /// 若多余 Exit 发生在他人在途期间，会把计数错误地减到 0，令 drain 提前成功 ⇒ 提前释放 ⇒ UAF。
    /// 因此 I3 独占性以「不存在多余 Exit」为前提。</para>
    /// <para>该前提由<b>穷举审计</b>而非运行时检测保证：本类为 <c>internal</c>，Enter/Exit 调用点有限且全部为
    /// <c>if (TryEnter()) { try { … } finally { Exit(); } }</c> 定式
    /// （MFDemuxer 3 处：OpenCore / ReadPacketCore / SeekAsync 的 lambda；
    /// MFVideoDecoder 4 处：DecodeAsync / RenegotiateOutput / FlushAsync / Reset。2026-08-01 逐处核验）。</para>
    ...
    /// <remarks>
    /// 语义与 <see cref="WaitDrain"/> 逐条对齐（审计要求）：已排空即刻 true（不分配状态机）；
